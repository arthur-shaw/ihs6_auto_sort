	...
	* unzip each file into a sub-folder with the name of the file (minus the ".zip" extension)
	foreach zipFile of local zipFiles {
		...
		* unpack zip file's contents into destination folder
		qui : cd "`folder'"
		* ! powershell.exe -NoP -NonI -Command "Expand-Archive '.\`zipFile'' '.\`filePath'\'"
		! powershell.exe -nologo -noprofile -command "& { Add-Type -A 'System.IO.Compression.FileSystem'; [IO.Compression.ZipFile]::ExtractToDirectory('`zipFile'', '`filePath''); }"
