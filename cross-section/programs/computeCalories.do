...
syntax ,					///
	...
	* drop cases with missing info
	drop if (unit_code == .)

	* try to handle missing factors with intelligent replacements
	replace `factorVar' = 1 if (unit_code == 1 & mi(`factorVar')) // kilograms
	replace `factorVar' = 0.001 if (unit_code == 18 & mi(`factorVar')) // grams
	replace `factorVar' = 0.300 if (unit_code == 31 & mi(`factorVar')) // loaf (300g)
	replace `factorVar' = 0.600 if (unit_code == 32 & mi(`factorVar')) // loaf (600g)
	replace `factorVar' = 0.700 if (unit_code == 33 & mi(`factorVar')) // loaf (700g)
	replace `factorVar' = 0.150 if (unit_code == 34 & mi(`factorVar')) // PACKET (150G)
	replace `factorVar' = 0.400 if (unit_code == 35 & mi(`factorVar')) // PACKET (400G)
	replace `factorVar' = 0.500 if (unit_code == 36 & mi(`factorVar')) // PACKET (500G)
	replace `factorVar' = 1 if (unit_code == 37 & mi(`factorVar')) // PACKET (1KG)
	replace `factorVar' = 0.250 if (unit_code == 65 & mi(`factorVar')) // PACKET (250G)
	replace `factorVar' = 0.250 if (unit_code == 65 & mi(`factorVar')) // PACKET (250G)
	replace `factorVar' = 0.025 if (unit_code == 70 & mi(`factorVar')) // PACKET (25G)
    replace `factorVar' = 0.100 if (unit_code == 71 & mi(`factorVar')) // TIN 100G   
    replace `factorVar' = 0.250 if (unit_code == 72 & mi(`factorVar')) // TIN 250G   
    replace `factorVar' = 0.500 if (unit_code == 73 & mi(`factorVar')) // TIN 500G   
	...
	* drop duplicates, if any exist
	...
	duplicates drop `prodID_fctrCurr' `unitIDs_fctrCurr' `geoIDs', force
	...
	* factors at most detailed level (e.g., by strata)
	tempfile factors
	save "`factors'"
		* save externally for review
		* save "C:\Users\wb393438\IHS5\auto-sort\cross-section\data\resource/factorsTest.dta", replace
